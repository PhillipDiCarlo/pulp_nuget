...
from pulp_nuget.app.models import NugetContent, NugetRemote
...
def synchronize(remote_pk, repository_pk, mirror):
    ...
    first_stage = NugetFirstStage(remote, deferred_download)
    DeclarativeVersion(first_stage, repository, mirror=mirror).create()


class NugetFirstStage(Stage):
    """
    The first stage of a pulp_nuget sync pipeline.
    """
    ...
    async def run(self):
        ...
        for entry in self.read_my_metadata_file_somehow(result.path):
            unit = NugetContent(entry)  # make the content unit in memory-only
    ...
    def read_my_metadata_file_somehow(self, path):
        """
        Parse the metadata for nuget Content type.
